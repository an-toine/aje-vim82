control "scl-runtime-package-is-installed-1.0" do
    ...
    describe package("aje-vim82") do
        it { should be_installed }
    end

    describe package("aje-vim82-runtime") do
        it { should be_installed }
    end
end
...
control "scl-dev-packages-are-installed-1.0" do
    ...
    describe package("aje-vim82-build") do
        it { should be_installed }
    end

    describe package("aje-vim82-scldevel") do
        it { should be_installed }
    end
end
...
control "scl-can-be-enabled-1.0" do
    ...
    describe file("/opt/aje/aje-vim82/enable") do
        its("mode") { should cmp "0755" }
        its("content") { should match %r{export PATH=".*"} }
    end

    describe bash("scl enable aje-vim82 'echo $PATH'") do
        its("stdout") { should match %r{/opt/aje/aje-vim82/root/usr/bin:/opt/aje/aje-vim82/root/usr/sbin:.*} }
