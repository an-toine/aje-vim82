control "vim-is-installed-1.0" do
    ...
    describe package("aje-vim82-vim") do
        it { should be_installed }
        its("version") { should match %r{8\.2.*} }
    end

    describe package("aje-vim82-vim-common") do
        it { should be_installed }
        its("version") { should match %r{8\.2.*} }
    end
end
...
control "gvim-is-installed-1.0" do
    ...
    describe package("aje-vim82-vim-gtk") do
        it { should be_installed }
        its("version") { should match %r{8\.2.*} }
    end
end
...
control "vim-binary-links-1.0" do
    ...
    for tool in ["vimdiff", "view", "rvim", "rview", "ex", "evim", "eview"]
        describe file("/opt/aje/aje-vim82/root/usr/bin/#{tool}") do
            its("type") { should eq :symlink }
            its("link_path") { should eq "/opt/aje/aje-vim82/root/usr/bin/vim" }
        end
    end

    describe file("/opt/aje/aje-vim82/root/usr/bin/vim") do
...
control "gvim-binary-links-1.0" do
    ...
    for tool in ["gvimdiff", "gview", "rgvim", "rgview"]
        describe file("/opt/aje/aje-vim82/root/usr/bin/#{tool}") do
            its("type") { should eq :symlink }
            its("link_path") { should eq "/opt/aje/aje-vim82/root/usr/bin/gvim" }
        end
    end

    describe file("/opt/aje/aje-vim82/root/usr/bin/gvim") do
        its("type") { should eq :file }
        its("mode") { should cmp "0755" }
    end
end
...
control "minimal-vim-is-working-1.0" do
    ...
    describe bash("scl enable aje-vim82 'vim --version'") do
...
control "gvim-is-working-1.0" do
    ...
    describe bash("scl enable aje-vim82 'gvim --version'") do
        its("exit_status") { should eq 0 }
    end
